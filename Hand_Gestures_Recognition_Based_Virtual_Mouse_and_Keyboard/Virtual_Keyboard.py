...
from pynput.keyboard import Key as keyy
...
show = False
cv2.namedWindow('Hand Gestures Recognition Based Virtual Mouse and Keyboard')
counter = 0
previousClick = 0
...
while True:
    ...
    frame = cv2.resize(frame, (950, 500))
    ...
    if lmList:
        ...
        if calculateIntDidtance((signTipX, signTipY), (thumbTipX, thumbTipY)) < 20:
    ...
    cv2.putText(frame, f'FPS:{int(fps)}', (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
    showKey.drawKey(frame, (255, 255, 255), (0, 0, 0), 0.1, fontScale=0.5)
    exitKey.drawKey(frame, (255, 255, 255), (0, 0, 0), 0.1, fontScale=0.5)
    cv2.setMouseCallback('Hand Gestures Recognition Based Virtual Mouse and Keyboard', getMousPos)
    ...
    if show:
            ...
            if k.isOver(mouseX, mouseY) or k.isOver(signTipX, signTipY):
                ...
                if k.isOver(thumbTipX, thumbTipY):
                    ...
                    if clickTime - previousClick > 0.4:
                        if k.text == '<--':
                            textBox.text = textBox.text[:-1]
                            keyboard.press(keyy.backspace)
                            keyboard.release(keyy.backspace)
                        elif k.text == 'clr':
                            textBox.text = ''
                        elif len(textBox.text) < 30:
                            if k.text == 'Space':
        ...
        clickedX, clickedY = 0, 0
    ptime = ctime
    cv2.imshow('Hand Gestures Recognition Based Virtual Mouse and Keyboard', frame)
